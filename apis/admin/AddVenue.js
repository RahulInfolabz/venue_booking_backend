...
async function AddVenue(req, res) {
  try {
    ...
    if (!occasion_id || !venue_type_id || !city_id || !description || !price) {
      return res.status(400).json({ success: false, message: "All fields are required" });
    }

    if (!ObjectId.isValid(occasion_id) || !ObjectId.isValid(venue_type_id) || !ObjectId.isValid(city_id)) {
    ...
    await db.collection("venues").insertOne({
      occasion_id: new ObjectId(occasion_id),
      venue_type_id: new ObjectId(venue_type_id),
    ...
    console.error("AddVenue.js: ", error);
